...
class ProfileHeader extends Component {
  render() {
    ...
    <p className="lead text-center">{profile.status} {isEmpty(profile.company) ? null : (<span>at {profile.company}</span>)}</p>
              <p>{isEmpty(profile.location) ? null : (<p>at {profile.location}</p>)}</p>
              <p>
                {/* checking to see if user has a website... */}
                ...
                {/* checking is user has instagram */}
